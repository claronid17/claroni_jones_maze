...
(def terminal-set
  '((wall-l gamestate) (wall-r gamestate) (wall-u gamestate) (wall-d gamestate)
                       (finish-l gamestate) (finish-r gamestate) (finish-u gamestate)
                       (finish-d gamestate) (rand-move gamestate)))
(defn rand-term
  "returns a ranom value in the terminal set"
  []
  (rand-nth terminal-set))
(take 2(generate-init-population 5))
